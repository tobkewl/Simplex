(() => {
if (window.__simplexBuildManagerInitialized) {
  console.warn('[BUILD_MANAGER] already initialized, skipping duplicate load')
  return
}
window.__simplexBuildManagerInitialized = true

const ipcRenderer = window.buildManagerAPI?.ipcRenderer
const appConfig = window.buildManagerAPI?.getPublicConfig?.() || {}
const { TreePreviewRenderer } = window
console.log('[BUILD_MANAGER] script loaded', {
...
function invokeWithTimeout(channel, ...args) {
  if (!ipcRenderer || typeof ipcRenderer.invoke !== 'function') {
    return Promise.reject(new Error('Build manager IPC bridge is unavailable.'))
  }
...
function resolveBuildTypeFromData(buildData) {
  const type = normalizeLookup(buildData?.build?.buildType || buildData?.build?.build_type)
  return type === 'live' ? 'live' : 'guide'
}
...
function renderSocketPreview(chain, container) {
  const sockets = normalizeArray(chain.sockets)
  const preview = createEl('div', 'socket-preview no-scrollbar')

  sockets.forEach((gem, idx) => {
    if (idx > 0) {
      const linkWrap = createEl('div', 'relative z-0 flex h-[52px] items-center')
      linkWrap.style.marginRight = `-${LINK_OVERLAP_RIGHT_PX_DEFAULT}px`
      const link = document.createElement('img')
      link.src = LINK_IMG
      link.alt = ''
      link.className = 'h-8 w-8 object-contain accent-glow'
      linkWrap.appendChild(link)
      preview.appendChild(linkWrap)
    }

    const socketWrap = createEl('div', 'relative z-10 flex w-[52px] flex-none items-center justify-center')
    socketWrap.style.marginRight = idx < sockets.length - 1 ? `-${LINK_OVERLAP_LEFT_PX_DEFAULT}px` : '0'

    const socket = createEl('div', 'relative w-[52px] h-[52px] rounded-full overflow-hidden')
    ...
    if (iconPath) {
      ...
      gemImg.src = iconPath
      gemImg.alt = gem?.name || ''
      gemImg.className = 'absolute inset-0 m-auto w-9 h-9 object-contain drop-shadow-[0_3px_8px_rgba(0,0,0,0.55)]'
...
function renderSkillChains(chains, container, options = {}) {
  ...
  chainList.forEach((chain, index) => {
    const card = createEl('div', 'skill-chain-card')
    ...
    header.appendChild(createEl('div', 'text-sm font-semibold', label))
    ...
    renderSocketPreview(chain, card)
    if (!options.compact) {
...
function renderLiveGuideView(blocks, gearByBlock, treeBySection, general, selectedBlockId = null) {
  ...
  renderSkillChains(filteredChains, skillsPanel)
...
function renderGuideSections(blocks, gearByBlock, notesByScope, guideExtraBlocksByBlockId, treeBySection, general, activeSectionId) {
  ...
  sections.forEach((block, index) => {
    ...
    const body = createEl('div', 'grid gap-4 lg:grid-cols-3')

    const skillsPanel = createEl('div', 'space-y-3')
    skillsPanel.appendChild(createEl('div', 'text-xs font-semibold uppercase text-muted-foreground', 'Skill Gems'))
    renderSkillChains(block.chains, skillsPanel, { compact: true })
    body.appendChild(skillsPanel)

    const gearPanel = createEl('div', 'space-y-3')
    gearPanel.appendChild(createEl('div', 'text-xs font-semibold uppercase text-muted-foreground', 'Gear'))
    ...
    const notesPanel = createEl('div', 'space-y-3')
...
function renderPaperdoll(slots) {
  const wrapper = createEl('div', 'flex justify-center overflow-x-auto')
...
function renderBuildsList() {
  ...
  list.forEach((build) => {
    ...
    header.appendChild(createEl('div', 'text-base font-semibold', toString(build?.name) || 'Untitled build'))
    ...
    if (isDiscover) {
      ...
      const date = createEl('div', 'mt-2 text-xs text-muted-foreground', `${dateLabel}: ${dateText}`)
...
async function loadBuilds() {
  try {
    ...
    if (!ipcRenderer || typeof ipcRenderer.invoke !== 'function') {
      throw new Error('Build manager IPC bridge is unavailable.')
    }
...
async function initialize() {
  if (!ipcRenderer || typeof ipcRenderer.invoke !== 'function') {
    renderBuildsListError('Build manager is not initialized correctly. Please restart the app.')
    return
  }
  ...
  if (userStatus) {
    const label = auth?.user?.poeAccountName || auth?.user?.name || auth?.username || auth?.email || 'Unknown'
    userStatus.textContent = `Logged in as ${label}`
  }
...
if (buildTabMine) {
  buildTabMine.addEventListener('click', () => setActiveBuildTab('mine'))
}
if (buildTabFollowing) {
  buildTabFollowing.addEventListener('click', () => setActiveBuildTab('following'))
}
if (buildTabDiscover) {
  buildTabDiscover.addEventListener('click', () => setActiveBuildTab('discover'))
}
...
initialize().catch((error) => {
  console.error('[BUILD_MANAGER] initialize failed:', error)
  renderBuildsListError(error?.message || 'Failed to initialize build manager.')
})
